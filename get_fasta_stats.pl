...
#
$Date_Last_Modified = "September 4, 2007";
...
use Getopt::Std;
$::opt_d = $::opt_g = $::opt_G = $::opt_h = $::opt_n = $::opt_p =
  $::opt_q = $::opt_s = $::opt_t = $::opt_T = $::opt_x = undef;
display_help('') unless( getopts('dgG:hnp:qstTx') );
...
display_help('Cannot use -q with -d or -x') if ($QUAL && $EXTENDED);
...
print_histogram() if $GRAPH_HISTOGRAM;
print STDOUT "\n";
...
sub print_headers
  ...
  if ($QUAL)
    ...
    print STDOUT "\nContig   \tContig\tConsed\tEdited\tPhred$PHRED_SCORE\tMinimum\tMaximum\t\tNonACGT\tLongest\n";
...
sub print_totals
  ...
  my $ave_len = sprintf "%.1f", $Total_Bases / $Num_Contigs;
  ...
  if ($QUAL)
    ...
    print STDOUT "Minimum Contig Quality=$Min_Qual, Maximum Contig Quality=$Max_Qual,\n";
...
sub print_histogram
  ...
  $length_counts{$_}++ foreach @sorted_lengths;
  ...
  print STDOUT "Median=$median, Mode=$mode occurs $mode_max times\n\n";
#printf STDOUT "length_counts=('%s'), %d\n\n", join("', '", map { "$_=$length_counts{$_}" } (@unique_lengths)), scalar keys %length_counts;
...
USAGE: $my_name [-d] [-g] [-G num_bars] [-n]
          [-q [-p phred_score]] [-s] [-t] [-T] [-x]
	  [fasta_input_file]
              or
...
OPTIONS:
  ...
  -d  Compute extended dna statistics on both strands.  -q may not be
      used with -x.  If both -d and -x are specified, then -d is used.
  ...
  -x  Compute extended dna statistics on the forward strands.  -q may
      not be used with -x.  If both -d and -x are specified, then -d
      is used.
